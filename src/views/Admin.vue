	...
	<div class="container">
		...
		<div style="width: 50%; padding-right: 30px; float: left">
			...
			<span class="input-group-btn">
				<button class="btn btn-success" type="button" @click="createUser(str)">
					추가
				</button>
			</span>
			...
			<ul class="list-group">
				<li
					...
					v-bind:key="index"
				>
					<strong>이름 : </strong>{{ user.name }} <strong>나이 : </strong>
					{{ user.age }} <strong>성별 : </strong> {{ user.sex }}
					<div
			...
			<button type="button" class="btn btn-warning" @click="pagePrev">
				◀ prev
			</button>
			<button type="button" class="btn btn-warning" @click="pageNext">
				next ▶
			</button>
		</div>
		...
		<div style="width: 50%; padding-left: 30px; float: right">
			<div class="btn-group">
				<button type="button" class="btn btn-primary" @click="searchOn">
					검색
				</button>
				<button type="button" class="btn btn-primary" @click="filterOn">
					필터링
				</button>
			</div>
			<div v-if="isSearch">
				...
				<span class="input-group-btn">
					<button class="btn btn-info" type="button" @click="searchUser(str)">
						검색
					</button>
				</span>
				<br />
...
export default {
	...
	created: function () {
		...
		for (var i = 0; i < this.maxpageNumber; i++) this.pageUsers.push([]);

		var page = 1;
		var num = 0;
		for (i = 0; i < this.usersNumber; i++) {
	...
	methods: {
		...
		pagePrev: function () {
			if (this.curpageNumber - 1 > 0) this.curpageNumber--;
		},
		pageNext: function () {
			if (this.curpageNumber + 1 <= this.maxpageNumber) this.curpageNumber++;
		},
	...
	watch: {
		...
		filterText: function () {
			...
			for (var i = 0; i < this.users.length; i++) {
				if (
					this.users[i].name === undefined ||
					this.users[i].age === undefined ||
					this.users[i].sex === undefined
				)
					continue;
				if (
					this.users[i].name === null ||
					this.users[i].age === null ||
					this.users[i].sex === null
				)
					continue;
				if (
					this.users[i].name.includes(this.filterText) ||
					this.users[i].age.includes(this.filterText) ||
					this.users[i].sex.includes(this.filterText)
				) {
